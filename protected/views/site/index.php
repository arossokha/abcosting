...
<div class="calc">
	<form method="post">
		Choose product <select class="mainSelect" name="wareId">
			<?php
				$selected = $_POST['wareId'] ?:'selected';
				echo "<option {$selected} disabled style='display:none;'>Select...</option>";
				foreach (Wares::model()->findAll() as $product) {
					$selected = ($product->primaryKey != $_POST['wareId']) ? :'selected';
					echo "<option {$selected} value='{$product->primaryKey}'>{$product->Name}</option>";
				}
			?>
		</select>
